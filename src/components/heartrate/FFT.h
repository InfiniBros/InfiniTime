...
namespace Pinetime {
  namespace Utility {
    ...
    public:
      ...
      static consteval auto GenComplexTwiddle_impl(std::index_sequence<I...>) {
        using namespace std::complex_literals;
        return std::array<std::complex<float>, IntegerLog2(N)> {
          std::complex<float>(exp_consteval(-2.i * std::numbers::pi / static_cast<double>(1 << (I + 1))))...};
      }

      template <std::size_t N>
      static consteval std::array<std::complex<float>, IntegerLog2(N)> GenComplexTwiddle() {
        return GenComplexTwiddle_impl<N>(std::make_index_sequence<IntegerLog2(N)>());
      }

      template <std::size_t N, std::size_t... I>
      static consteval auto GenRealTwiddle_impl(std::index_sequence<I...>) {
        using namespace std::complex_literals;
        return std::array<std::complex<float>, (N / 4) - 1> {
          std::complex<float>(exp_consteval(-2.i * std::numbers::pi * static_cast<double>(I + 1) / static_cast<double>(N)))...};
      }

      template <std::size_t N>
      static consteval std::array<std::complex<float>, (N / 4) - 1> GenRealTwiddle() {
        return GenRealTwiddle_impl<N>(std::make_index_sequence<(N / 4) - 1>());
      }
